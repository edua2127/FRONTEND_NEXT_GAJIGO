                ...
                <main>
                    <section className={style.room_section}>
                        <article className={style.room_article_cadastro_and_listar}>
                            <button onClick={() => Router.push('/auth/cadastrarTags')} className={style.room_button_cadastrar}>Cadastrar</button>
                        </article>
                        <article className={style.room_article_table}>
                            <table className={style.room_table}>
                                ...
                                {tags.length > 0 && tags.map((tag) => {
                                    ...
                                    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                                    // @ts-ignore
                                    const idLocal:string = tag._links.self.href.split('/').pop()

                                    return (
                                        <tr key={idLocal} className={style.room_table_tr}>
                                            <td className={style.room_table_td}>{tag.name}</td>
                                            <td className={style.room_table_td}>{tag.description}</td>
                                            <td className={style.room_table_td}>{tag.created.toString()}</td>
                                            <td className={style.room_table_td_actions}>
                                                <button className={style.room_button_editar}
                                                    onClick={() => Router.push(`/editarTags/${idLocal}`)}>Editar
                                                </button>
                                                <button className={style.room_button_excluir} onClick={() => excluirTag(idLocal)}>Excluir</button>
                                            </td>
                                        </tr>
                                    )
                                })}
