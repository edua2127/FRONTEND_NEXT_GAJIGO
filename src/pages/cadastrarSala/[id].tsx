...
const CadastrarSala: NextPage = () => {
    ...
    function cadastrar() {
        RoomService.create(room).then((response) => {
                    ...
                    <section className={style.cadastro_events_section}>
                        <article className={style.cadastro_events_article}>
                            <label className={style.cadastro_events_label}>
                                ...
                                <input type="text" value={room.name} className={style.cadastro_events_input}
                                       onChange={(e) => setRoom({...room, name: e.target.value})}/>
                            </label>
                            <label className={style.cadastro_events_label}>
                                <span>Descrição da Sala</span>
                                <input type="text" value={room.description} className={style.cadastro_events_input}
                                       onChange={(e) => setRoom({...room, description: e.target.value})}/>
                            </label>
                        </article>
                        <article className={style.cadastro_events_article_button}>
                            <button className={style.cadastrar_sala_button_cancelar} onClick={()=> Router.back()}>Cancelar</button>
                            <button className={style.cadastrar_sala_button} onClick={cadastrar}>Cadastrar</button>
                        </article>
                    </section>
                </main>
            </>
